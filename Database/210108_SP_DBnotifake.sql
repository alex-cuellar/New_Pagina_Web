...
`usuario`.`Imagen`,
`comentarios`.`Fecha`,
`comentarios`.`IsRegister`,
`comentarios`.`UsrAnonimo`,
`comentarios`.`Calif`,
`comentarios`.`IsHidden`
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ComentarioMeGusta`(
IN `IDN` int
)
BEGIN
UPDATE `notifakedb`.`comentarios`
SET
`Calif` = `Calif` + 1
WHERE `ID` = `IDN`;
END$$
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ComentarioNoMeGusta`(
IN `IDN` int
)
BEGIN
UPDATE `notifakedb`.`comentarios`
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Buscar_Noticia`(
IN word1 varchar(500)
)
BEGIN

SELECT *
FROM `noticia`
WHERE `noticia`.`Titulo` LIKE CONCAT('%', word1, '%')
AND `noticia`.`Aceptada` = 1;

END$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Comentario_Anonimo`(
IN PIdNoticia int,
IN PIdUsr int,
IN PTexto varchar(500),
IN AnonimoN varchar(100)
)
BEGIN
INSERT INTO `notifakedb`.`comentarios`
(`IdNoticia`,
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `OBTENER_COMENTARIO`(
IN `ID` int)
BEGIN
SELECT 
`comentarios`.`ID`,
